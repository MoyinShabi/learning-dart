void main() {
  ...
  final colors2 = [
    'grey',
    'brown',
    if (addBlue == true) 'blue', //`Collection-if` which can be used directly in a list literal unlike the normal `if` statement above.
    if (addRed) 'red', //`Collection-if` also
    for (var color in extraColors) color, //`Collection-for` which can be used instead of the `addAll` method above.
    ...moreColors, //`Spread, ...` which adds the elements of a list to the enclosing list just like a `collection-for`, but much easier.
    if (addMoreColors) ...['white', 'black'], //Shows that the collection-if, collection-for, and spread are composable.
  ];
  print(colors2); //[grey, brown, red, yellow, green, purple, orange, white, black]
  ...
  print(restaurant); //{name: Pizza Hut, cuisine: Italian, ratings: [4.0, 4.5, 3.5], isPopular: true}
}
